...
✔ /path/to/pdf-image-folder/output/page-0001.json
...
pdf_to_images 返回的是按页排序的页面图片路径列表。你必须自己按这个列表顺序推进：

1. 取下一张尚未完成的页面图片。
2. 调用 pdf_ocr(image_path, page, page_image)。
3. pdf_ocr 返回后，立刻调用 write_text_file(path, json) 写入该页 JSON。
4. write_text_file 成功后，才允许处理下一张页面图片。

严禁在 write_text_file 成功前开始下一页，也严禁重复处理已经完成 OCR 并写入 JSON 的页面。

例如处理 page-0001.png：

1. 调用 pdf_ocr(image_path="/.../page-0001.png", page=1, page_image="page-0001.png")
2. pdf_ocr 返回页级 JSON
3. 将返回 JSON 原样用 write_text_file 写入页面图片文件夹下的 output/page-0001.json，例如 /.../DemoPdf/output/page-0001.json
4. 每次只能调用一页 OCR，不能一次性处理多页。

输出路径规则：

- path 必须位于当前页面图片所在文件夹下的 output/ 子目录。
- 文件名必须使用 4 位页码：page-0001.json、page-0002.json、page-0003.json。
- 示例：image_path="/a/b/DemoPdf/page-1.png", page=1 时，write_text_file 的 path="/a/b/DemoPdf/output/page-0001.json"。

禁止通过任何文本工具读取 page-0001.png 的文件字节。

每页输出 JSON 文件：

页面图片文件夹/output/page-0001.json
